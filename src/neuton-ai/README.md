 ...
 * `neuton_generated` - folder containing all user solution-specific files (`neuton_user_model.c/h`).

1. Link Neuton C static library for your CPU architecture to your firmware project.
2. Add `./neuton/include` folder to your project global INCLUDE paths.
3. **Optional** Add folder where  `./neuton_generated` **is located** (not this folder itself) to your project global INCLUDE paths.
...
```
For making an inference with Neuton, there are only 4 functions that you should use:
* `neuton_nn_user_model` - Get user model pointer;
* `neuton_nn_setup` - Set up the internal components of Neuton user model, **should be called first and once**;
...
static neuton_nn_t* p_nn = NULL;

void user_init_neuton_model(void)
{
    // Get user model pointer
    p_nn = neuton_nn_user_model();
    // Setup Neuton library, should be called once!
    neuton_status_t res = neuton_nn_setup(p_nn);
    // Optional check for success, #include <assert.h> required
    assert(res == NEUTON_STATUS_SUCCESS);
}
//
// ....
//
...
{
    uint32_t number_of_features = neuton_nn_uniq_inputs_num(p_nn); // Will be equal to 3
    // Feed raw input features to prepare them for model inference
    neuton_status_t res = neuton_nn_feed_inputs(p_nn, raw_features, number_of_features);
...
void user_feed_data_to_model(void)
{
    ...
    uint32_t number_of_features = neuton_nn_uniq_inputs_num(p_nn); // Will be equal to 3
    uint32_t window_size_of_features = neuton_nn_input_window_size(p_nn); // Will be equal to N
    neuton_input_t* p_raw_features_begin = &raw_features[0];

    for (size_t i = 0; i < window_size_of_features; i++)
    {
        neuton_status_t res = neuton_nn_feed_inputs(p_nn, p_raw_features_begin, number_of_features);
...
```
Inference results you can get from `p_nn->decoded_output` structure:
- `p_nn->decoded_output.classif` - for classification models.
- `p_nn->decoded_output.regression` - for regression models.
- `p_nn->decoded_output.anomaly` - for anomaly detection models.
...
void user_feed_data_to_model(void)
{
    ...
    neuton_status_t res = neuton_nn_feed_inputs(p_nn, raw_features, 
                                            neuton_nn_uniq_inputs_num(p_nn) * 
                                            neuton_nn_input_window_size(p_nn));

    // Check if input data is prepared and ready for model inference
    if (res == NEUTON_STATUS_SUCCESS)
    {
        // Run model inference
        res = neuton_nn_run_inference(p_nn);
        // Check if model inference is ready and successful
        if (res == NEUTON_STATUS_SUCCESS)
        {
            neuton_u16_t predicted_class = p_nn->decoded_output.classif.predicted_class;
            size_t num_classes = p_nn->decoded_output.classif.num_classes;
            // Get probability depending on model quantization: f32, q16, q8. Here is an example for f32 model
            const neuton_f32_t* p_probabilities = p_nn->decoded_output.classif.probabilities.p_f32;
...
void user_feed_data_to_model(void)
{
    ...
    if (res == NEUTON_STATUS_SUCCESS)
    {
        ...
        if (res == NEUTON_STATUS_SUCCESS)
        {
            const neuton_f32_t* p_predicted_values = p_nn->decoded_output.regression.p_outputs;
            size_t values_num = p_nn->decoded_output.regression.outputs_num;
...
void user_feed_data_to_model(void)
{
    ...
    if (res == NEUTON_STATUS_SUCCESS)
    {
        ...
        if (res == NEUTON_STATUS_SUCCESS)
        {
            neuton_f32_t anomaly_score = p_nn->decoded_output.anomaly.score;
...
* `neuton_nn_user_model_size` - Get user model size in bytes (flash usage)